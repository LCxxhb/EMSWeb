var planPage = {
	init: function() {
		...
		planPage.LoadOneSelect();//加载分厂
		planPage.loadParentMediaSlect();//加载介质
	},
 ...
 //加载页面二级介质下拉框
 loadChildMediaSlect:function(){
     $("#mediaEr").empty(); //重置下拉框
     var pid = $.trim($('#mediaYi option:selected').val()); //获取选中的区域
     if(pid==""){
   ...
   //加载页面分厂一级下拉框
	...
	//加载页面区域二级下拉框
	loadTwoSlect:function(){
		$("#areaName").empty(); //重置下拉框
		var aid = $.trim($('#factory option:selected').val()); //获取选中的区域
		if(aid==""){
			layer.alert("请先选择分厂！")
			return;
		}
