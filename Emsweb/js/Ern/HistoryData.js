...
var Erhistory = {
	...
	findAllData: function() {
		...
		var url;
		if (type == 1) {
			url = "water/findAllHistory";
		} else if (type == 2) {
			url = "";
		} else if (type == 3) {
			url = "gas/findAllHistory";
		}
		Ter.getApi({
				apiname: url
			},
	...
	eleReportSearch: function() {
		...
		//var jzType = $("#jzType").val(); //介质
		// var ErFactory = $("#ErFactory").val(); //分厂
		// var ErArea = $("#ErArea").val(); //区域
		var startDayDate = $("#startDayDate").val(); //开始时间
		var endDayDate = $("#endDayDate").val(); //结束时间
		//把查询条件放入参数对象
		var params = {
			"param1": ErFactory, //Parma1;分厂名
			"param3": jzType, //介质名称
			"param2": ErArea, //区域
			"param6": jzleix, //介质类型
